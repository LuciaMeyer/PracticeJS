...
const countPackages = (carriers, carrierID) => {
    let num = 0;
    let first = carriers.find(arr => arr[0] === carrierID);
    if(first) {
        if(first[2].length !== 0) {
            num = first[1] + countPackages(carriers, first[2][0]) + countPackages(carriers, first[2][1]);
        } else num = first[1]
    }
    return num;
};
...
console.log(countPackages(carriers1, 'dapelu'));
